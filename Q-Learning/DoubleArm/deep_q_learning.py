            ...
            for step in range(self.MAX_STEPS):
                ...
                if random_number < epsilon: 
                    if random_number < epsilon/2:
                        ...
                        try:
                            left_dist = np.linalg.norm(self.goal.get_position() - self.left_arm.base_pos)
                            right_dist = np.linalg.norm(self.goal.get_position() - self.right_arm.base_pos)
                            if left_dist < right_dist: # goal is closer to left arm
                                u = self.left_arm.get_control(self.goal.get_position() - self.left_arm.get_position())
                                ...
                                u = self.right_arm.get_control(self.goal.get_position() - self.right_arm.get_position())
                                if np.argmax(np.abs(u)) == 0:
                                    if u[0] < 0:
                                        action = 4
                                    else:
                ...
                #FIXME: removing all plotting in this module will double the framerate
                #TODO: consider using the GUI for plotting (in a future function of pyglet_draw)
