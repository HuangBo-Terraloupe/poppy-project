...
JOINT_LIMITS['right'] = ((20./180*np.pi, 165./180*np.pi, ), (-1./180*np.pi, 148./180*np.pi)) # RIGHT ARM 

class Goal_Arm:
    def __init__(self, arm_length1, arm_length2, offset=10.):
        ...
        x = np.abs(arm_length1*np.cos(angle1) + arm_length2*np.cos(angle1+angle2))
        y = offset + arm_length1*np.sin(angle1) + arm_length2*np.sin(angle1+angle2)
