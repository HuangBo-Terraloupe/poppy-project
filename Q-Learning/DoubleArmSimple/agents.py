...
class Arm:
    def __init__(self, side='left', base_pos=[0.0, 0.0], angular_velocity_1=ANGULAR_ARM_VELOCITY, angular_velocity_2=ANGULAR_ARM_VELOCITY, arm_length_1=ARM_LENGTH_1, arm_length_2=ARM_LENGTH_2):
    ...
    def get_normalized_angles(self):
        ...
        range1_half = 0.5*(JOINT_LIMITS[self.SIDE][0][1] - JOINT_LIMITS[self.SIDE][0][0])
        theta1 = (self.theta[0] - range1_half)/range1_half
        range2_half = 0.5*(JOINT_LIMITS[self.SIDE][1][1] - JOINT_LIMITS[self.SIDE][1][0])
    ...
    def update(self):
        ...
        self.theta[0] = np.maximum(np.minimum(self.theta[0], JOINT_LIMITS[self.SIDE][0][1]), JOINT_LIMITS[self.SIDE][0][0])
        self.theta[1] = np.maximum(np.minimum(self.theta[1], JOINT_LIMITS[self.SIDE][1][1]), JOINT_LIMITS[self.SIDE][1][0])
