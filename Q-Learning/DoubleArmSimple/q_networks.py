...
class QNetworks:
    def __init__(self, num_of_actions, num_of_states, num_of_hidden_neurons=NUM_OF_HIDDEN_NEURONS, tau=TAU, network_name=QNETWORK_NAME): 
        self.NUM_OF_ACTIONS = num_of_actions
        self.NUM_OF_HIDDEN_NEURONS = num_of_hidden_neurons
        self.NUM_OF_STATES = num_of_states
        self.TAU = tau

        self.online_net = self.init_model(network_name)
        self.target_net = self.init_model(network_name)
    ...
    def save_models(self):
        #FIXME: this seems to take some time and slows down training and rendering (running inside learner thread and should not slow down rendering)
        weights = self.online_net.get_weights()
        for i in xrange(len(weights)):
            np.savetxt(QNETWORK_NAME+'/'+QNETWORK_NAME+str(i)+'.txt', weights[i])
        print("Saved online model to disk.")